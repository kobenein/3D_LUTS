...
def CreateModel(res=True, nfs=[4,8,16,24], KI="glorot_uniform"):
    ...
    x = Conv2D(nfs[0], (1, 1), kernel_initializer=KI, activation='relu')(input_pix)

    for nf in nfs[1:]:
        # print(nf)
        x = Conv2D(nf, (1, 1), kernel_initializer=KI, activation='relu')(x)

        blockInput = x
        x = Conv2D(nf, 1, 1, kernel_initializer=KI, activation=tf.keras.activations.relu)(blockInput)
    ...
    x = Conv2D(nfs[-1], (1, 1), kernel_initializer=KI, activation='relu')(x)
    ...
    output_pix = Conv2D(3, (1, 1), kernel_initializer=KI, activation='relu')(x)
...
def maxSAD(y_true, y_pred):
    y_true = tf.clip_by_value(tf.round(y_true+128), 0, 255)
    y_pred = tf.clip_by_value(tf.round(y_pred+128), 0, 255)
